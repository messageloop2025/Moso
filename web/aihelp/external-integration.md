...
**斜杠 Command / Hook**：网页聊天以 `/skill-name [args]` 开头可强制加载 Skill 全文（`{{arg}}` 占位；`commands/*.md` 别名）；Skills 页可配置斜杠名、`hooks.json`、matcher 与匹配后决策（ask/deny/allow）。集成通道无确认弹窗时，Hook/`strict` 要求确认的操作会被拒绝。
