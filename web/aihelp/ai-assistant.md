...
- **语言优先级**（与后端 `resolve_output_language` 一致）：用户在本条消息里**显式**要求某种语言（如「用英文答」）优先 → 否则尽量与**本条用户输入**的主要语言一致 → 若无法判断，则依次采用 **「模型配置」中的「回复语言」** → **站点全局「回复语言」** → **当前界面语言**（「显示语言」选「跟随浏览器」时由浏览器/系统决定；否则会固定为你在设置里选的 zh-CN 或 en）。
...
### Skills：Hook 与斜杠 Command（给用户与 AI）

须管理员开启 Skills。AI **可以且应当**按你的要求创建带这些特性的 Skill（调用工具落地，不是只给文案）。
...
- **严格**（平台门禁，不加重模型负担）：
  ...
  - 写类工具（如 `ssh_execute`、`send_to_terminal`、`ssh_channel_send`）在真正执行前，由平台弹出**独立确认模态**（非聊天内选择题）：**允许** / **总是** / **拒绝**。
  - **总是**：本会话内**同一工具函数名**后续不再弹窗（不是按完整命令精确匹配）。切出严格模式后缓存保留但不使用；切回仍有效。
  - **不弹确认**：`connect_terminal`、`create_console`、`create_local_console`、`ssh_channel_create`（创建/打开终端与通道可直接进行）。
  - 用户点选后，tool 返回中带 `user_decision` / `user_decision_note`（已批准 / 未批准等），供模型决定继续或改问用户；措辞不强调「严格模式」。
  - 集成 / OpenClaw 等无确认 UI 的通道：需确认的操作会被拒绝。
- **位置**：全局 AI / 主机 AI / 本机 AI **顶部工具条**，紧挨「模型」下拉右侧（仅下拉框，无「模式」标签文字）。

## 低交互与运行中控制

### 低交互开关

- **位置**：AI 助手、主机 AI 运维、本机管理 AI 的聊天输入区域都有会话级开关，中文显示为「低交互」，英文显示为 `Low interaction`。
...
- **焦点行为**：运行中控制条出现时会自动聚焦补充输入框；AI 完成后自动回到普通聊天输入框；若补充框里还有未发送的文字，会自动移到聊天输入框，避免丢失。

### 终端轮询等待（唤醒 / 停止）

当 AI 通过 **Web 控制台**轮询长任务（编译、下载、安装等）时，可能在 `get_terminal_buffer` 后进入**服务端 batch 末倒计时**（由 `next_poll_in_seconds` 或自动推断决定，最长 3600 秒）。

- **在哪里看**：聊天右侧 CoT（思考与计划）里，**触发 batch 末等待的那一步工具**（通常是「第 N 轮 · 调用 get_terminal_buffer」）下方会出现橙色条：**剩余 Ns · get_terminal_buffer**。
- **唤醒**：认为已经等够、或想立刻再看终端输出时点击 **唤醒**。AI 会**马上进入下一轮**，不会把整个任务停掉。
- **停止**：要取消当前 AI 任务时点击该步骤上的 **停止**（效果同底部「停止」）。
- **条件返回 `until_contains`**：工具在超时内轮询至子串或超时；**不会**出现橙色倒计时条（等待在工具内完成），但仍可被 runtime **唤醒/停止**打断。
- **与 ssh_channel 的区别**：通道用显式 **`wait_seconds=1～30`**（0=立即）做 batch 末短等待，或 **`until_contains` + 超时**；不做自动长等待推断。Web 控制台用 `next_poll_in_seconds`（可达 3600）或同样的 `until_contains`。
...
- **系统设置**（管理员全局项、显示语言、个人发信等）：普通用户主要改显示语言与个人 SMTP；**不再**在此页配置 AI Key/模型。
- **模型配置**（侧栏 `/model-config`）：每个用户可维护**多组**命名配置（Profile），并指定**当前使用**的一组。字段包括 **模型类型**（阿里云 DashScope / Ollama / OpenAI 或自定义）、服务地址与模型、**上下文长度**、系统提示词、自动审批、**支持图像识别**、回复语言等。AI 助手、主机 AI、定时/触发任务均使用**当前激活** Profile。
- **快速切换**：在 AI 聊天栏可通过 Profile **下拉框**切换激活配置（悬停 title 提示），无需离开对话页。
- **模型身份（system 注入）**：每轮对话后端会在 system prompt 中注入「当前运行时模型身份」（激活 Profile、model id、provider、上下文预算、识图开关等）。用户问「你是什么模型 / 上下文多大」时，AI 应依据该块如实回答，而非猜测。
- **导入 / 导出**：支持 JSON 备份全部或单条配置；导入时可选择「仅新增（同名跳过）」或「覆盖同名」。
- **未配置自有 API Key 时（系统共享 Key 配额）**：若当前激活 Profile 尚未填写 API Key，系统会**自动使用管理员配置的系统 KEY** 代为调用，并计入该用户的**调用次数上限**（默认 **2000 次**）。**一旦 Profile 中配置了自有 KEY，则不再计入该上限。** 在使用系统共享 Key 期间：
  - 每一轮对话中，AI **首条 assistant 回复**开头会插入一行横幅，提示**系统共享 Key 配额**、已用次数与剩余额度；
  - **「模型配置」**页面顶部同步显示额度进度；
  - 达上限后，AI 不再调用模型，而是自动回复固定说明，引导用户在「模型配置」中填写自有 Key，或联系管理员处理配额。
- **管理员如何重置配额或改为自有 Key**：
  - **UI（推荐）**：管理员进入「用户管理」，在目标用户行点击 **「AI 配额」**，可查看该用户已用/剩余次数、是否已配置自有 KEY，常见操作包括：
    - **重置次数**：将该用户**系统 KEY** 调用计数清零；
    - **解除限制（应用系统默认配置到用户）**：把系统默认 AI 配置**写入该用户当前激活 Profile**，其后不再受系统 Key 次数限制。
  - **AI 技能**：`reset_user_system_ai_usage`、`apply_system_ai_config_to_user`。
- 更详细的逐步说明见 [model-config.md](model-config.md)。
