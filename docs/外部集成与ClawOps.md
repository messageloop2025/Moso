...
- **工具数量**：**47** 个 `edgeops_*`（含 claw-ops 同名 22 个 + MCP 扩展 25 个）
...
| 能力 | claw-ops | 毛竹 MCP |
|------|----------|-------------|
| 工具数 | **22 核心 + manifest 动态扩展 + invoke**（baseline **43**） | **47**（超集） |
...
| **`/integration/mcp/*`** | **MCP 专用**（需 `X-EdgeOps-Client: mcp`）：ssh_execute、编排、remote_fs 等 |
