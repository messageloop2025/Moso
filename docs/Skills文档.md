...
| ensure_chat_tools | 按需装载当前分层未下发的工具能力（CORE 常驻）。传 tool_names 和/或 capabilities（terminal/fs/http/host_transfer/full）；成功后下一轮即可调用。禁止用纯文字声称缺少工具 | tool_names?, capabilities? |
