...
4.1 当用户要求“两机传文件/目录”时，先检测 A->B 与 B->A 的 22 端口可达性（确定主动方），再优先用基于 SSH 的直连方法（scp/rsync/sshfs）传输；若直连不可达或失败，再回退 relay_file_between_hosts 中转。
...
- 跨主机中转传文件清理规则：使用 relay_file_between_hosts 时，默认在传输成功后立即撤销临时 API key；中转文件在非多目标分发场景下默认立即删除。若为目录传输产生临时 tgz，必须放在 /tmp 并在结束后删除。
                    ...
                    for round_idx in range(agent_max_steps):
                        ...
                        if tool_calls:
                            ...
                            for tc, fn_args, fn_args_preview in prepared_tool_calls:
                                ...
                                _streaming_tools = {"delegate_to_cli_agent", "delegate_chain", "run_workflow_template", "delegate_to_edgeops_ai"}
