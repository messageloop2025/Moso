...
_USER_SKILLS_HINT = (
    ...
    "- **Skills 分组**（库内元数据，与主机分组无关）：`list_user_skill_groups`、"
    "`create_user_skill_group`、`update_user_skill_group`、`delete_user_skill_group`、"
    "`assign_user_skills_to_group`；`save_user_skill` 可传 `group_name`/`group_id`\n\n"
