...
_USER_SKILLS_HINT = (
    ...
    "- **参数自动补全约定（创建 Command 时必须遵守）**：聊天填参浮层会从 Skill 抽取建议 chips。"
    "有固定子命令/常用参数时，**务必**在 frontmatter 或正文声明，否则用户只能看到「暂无内置参数建议」。\n"
    "  · 推荐 frontmatter：`slash-args: [balance, ecs list, oss ls]`（或 `slash_args` / 逗号分隔字符串）。\n"
    "  · 或正文小节 `## 斜杠参数` / `## 用法` / `## Commands` 下用列表项：`- balance — 查询余额`。\n"
    "  · 或写示例行如 `/my-cmd balance`；或 HTML 注释 `<!-- slash-args: a, b -->`。\n"
    "  · `commands/<alias>.md` 同样可写 `slash-args` / 斜杠参数列表，供 `/alias` 填参提示。\n"
    "- 子命令：`write_user_skill_file(name, path=\"commands/check-disk.md\", content=\"...\")`，"
    "用户可用 `/check-disk host1` 唤起（同样支持占位符与参数建议）。\n"
    "- 可选 `allowed_tools`：仅当用户**本轮斜杠唤起**该 Skill 时强制白名单（如 `ssh_execute,list_hosts`）。\n\n"
    "**Hook（须会创建）**：\n"
    "- 作用：在 AI 调工具前/后由平台拦截（与聊天「严格模式」正交；`ask` 弹独立确认框）。\n"
    "- 方式：写 hooks.json 文件 → `write_user_skill_file(..., path=\"hooks.json\", content=...)`；\n"
    "  或 `save_user_skill(..., hooks_enabled=true, hooks_json=<JSON字符串>)`。\n"
    "  事件：`preToolUse` / `postToolUse` / `postToolUseFailure` / `sessionStart` / `sessionEnd` / `beforeMCPExecution`；\n"
    "  decision=`allow`|`deny`|`ask`|`eval`。\n"
    ...
    "   hooks_enabled=true, allowed_tools=\"ssh_execute,get_terminal_buffer,list_hosts\",\n"
    "   hooks_json='{\"preToolUse\":{\"matcher\":\"ssh_execute\",\"decision\":\"ask\",\"reason\":\"……\"}}',\n"
    "   content=\"---\\nname: safe-ssh\\ndescription: ...\\nslash-args:\\n  - host1\\n  - web-01\\n---\\n"
    "# Safe SSH\\n\\n目标：`{{arg}}`\\n\\n## 斜杠参数\\n- `host1` — 示例主机\\n\\n步骤…\")`\n"
    "2) 需要细规则时再 `write_user_skill_file` 写 `hooks.json` 或 `commands/*.md`（子命令也写 slash-args）。\n"
