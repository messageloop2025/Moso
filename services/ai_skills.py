    ...
    try:
        ...
        if name == "list_available_events":
            ...
            _desc_map = {
                ...
                "agent:llm:pre": "LLM 调用前",
                "agent:llm:post": "LLM 返回后",
                "agent:llm:chunk": "LLM 流式片段（仅在 content/reasoning chunk 时触发）",
                "agent:token": "Token 用量统计（LLM 返回 usage 时触发）",
                "agent:turn:start": "每轮对话开始（LLM 调用前）",
                "agent:turn:end": "每轮对话结束（工具执行完成或本轮中断）",
                "session:create": "会话创建",
                "session:delete": "会话删除",
            ...
            _active_events = {
                ...
                "agent:step:start", "agent:step:end",
                "agent:tool:pre", "agent:tool:post", "agent:tool:error",
                "agent:llm:pre", "agent:llm:post", "agent:llm:chunk", "agent:token",
                "agent:turn:start", "agent:turn:end",
                "session:create", "session:delete",
                "mcp:pre",
            }
